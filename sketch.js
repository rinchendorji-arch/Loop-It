...
function clearHand(h){
  ...
  pState[h]=[false,false,false,false]; pDist[h]=[1,1,1,1];
...
function draw(){
  ...
  for(let i=particles.length-1;i>=0;i--){const p=particles[i];p.x+=p.vx*(1+beatP*.3);p.y+=p.vy*(1+beatP*.3);p.vy+=.14;p.vx*=.97;p.life-=1.7;if(p.life<=0){particles.splice(i,1);continue;}noStroke();fill(red(p.c),green(p.c),blue(p.c),p.life*2.2);circle(p.x,p.y,p.r*2);}
